...
export default ruleCreator({
    meta: {
        ...
        schema: [],
        messages: {
            avoidStyleAssignment: "Avoid setting styles directly via `{{property}}`. Use CSS classes for better theming and maintainability. Use the `setCssProps` function if the CSS properties need to change dynamically.",
        },
    },
    defaultOptions: [],
    create(context) {
        return {
            ...
            AssignmentExpression(node) {
                ...
                if (left.type === TSESTree.AST_NODE_TYPES.MemberExpression &&
            ...
            CallExpression(node) {
                ...
                if (propertyName === "setCssProps" && node.arguments[0].type === TSESTree.AST_NODE_TYPES.ObjectExpression) {
                ...
                if (propertyName === "setCssStyles" && node.arguments[0].type === TSESTree.AST_NODE_TYPES.ObjectExpression) {
                    for (const property of node.arguments[0].properties) {
                        if (property.type === TSESTree.AST_NODE_TYPES.Property && property.key.type === TSESTree.AST_NODE_TYPES.Literal && typeof property.key.value === 'string' && !property.key.value.startsWith('--')) {
                            context.report({
                                node,
