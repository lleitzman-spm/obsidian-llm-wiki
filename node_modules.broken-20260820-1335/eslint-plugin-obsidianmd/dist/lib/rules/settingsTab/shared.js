 ...
 */
export function isPluginSettingTab(node) {
    return (node.superClass?.type === TSESTree.AST_NODE_TYPES.Identifier &&
 ...
 */
export function findClassMember(node, name) {
    for (const member of node.body.body) {
        if (member.type !== TSESTree.AST_NODE_TYPES.MethodDefinition &&
        ...
        if (key.type === TSESTree.AST_NODE_TYPES.Literal &&
            key.value === name) {
            return member;
        }
    }
 ...
 */
export function resolveMinAppVersion(context) {
