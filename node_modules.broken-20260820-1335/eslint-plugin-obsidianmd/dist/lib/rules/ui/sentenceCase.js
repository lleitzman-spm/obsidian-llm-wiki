...
function isPropertyWithKey(prop, keyName) {
    ...
    if (prop.key.type === TSESTree.AST_NODE_TYPES.Literal)
    ...
    for (const prop of unwrapped.properties) {
        ...
        if (keyName === "attr" && valueNode.type === TSESTree.AST_NODE_TYPES.ObjectExpression) {
            for (const attrProp of valueNode.properties) {
                if (attrProp.type !== TSESTree.AST_NODE_TYPES.Property)
                    continue;
...
function visitStatementTree(statement, handleReturn) {
    ...
    if (statement.type === TSESTree.AST_NODE_TYPES.ForStatement ||
...
export default ruleCreator({
    ...
    create(context) {
        ...
        function checkGetDisplayTextReturns(node) {
            if (node.key.type !== TSESTree.AST_NODE_TYPES.Identifier || node.key.name !== "getDisplayText")
                return;
            const fn = node.value;
            if (!fn || fn.type !== TSESTree.AST_NODE_TYPES.FunctionExpression || !fn.body)
                return;
        ...
        return {
            ...
            AssignmentExpression(node) {
                if (node.left.type !== TSESTree.AST_NODE_TYPES.MemberExpression || node.left.property.type !== TSESTree.AST_NODE_TYPES.Identifier)
                    return;
