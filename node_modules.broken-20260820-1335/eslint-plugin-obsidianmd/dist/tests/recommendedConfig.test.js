...
async function rulesFor(configName, filename) {
...
function getSeverity(ruleValue) {
    if (typeof ruleValue === "string")
        return ruleValue;
    if (Array.isArray(ruleValue)) {
        if (typeof ruleValue[0] === "string")
            return ruleValue[0];
        if (ruleValue[0] === 0)
            return "off";
        if (ruleValue[0] === 1)
    ...
    describe("structure", () => {
        ...
        it("should have valid severities for all resolved rules", () => {
            for (const [rule, value] of Object.entries(tsRules)) {
                ...
                assert.ok(VALID_SEVERITIES.has(severity), `${rule} has invalid severity: ${severity}`);
            }
        });
        it("every registered obsidianmd rule should appear in the .ts config", () => {
            ...
            for (const rule of registeredRules) {
                ...
                if (fileSpecificRules.has(prefixed))
                    continue;
                assert.ok(prefixed in tsRules, `registered rule ${prefixed} is not referenced in the recommended config for .ts files`);
            }
        });
        it("type-checked rules should not appear in JS config", () => {
            for (const rule of TYPE_CHECKED_RULES) {
                assert.ok(!(rule in jsRules), `type-checked rule ${rule} should NOT be in the JS config (would crash without type info)`);
            }
        });
        it("type-checked rules should appear in TS config", () => {
            for (const rule of TYPE_CHECKED_RULES) {
                assert.ok(rule in tsRules, `type-checked rule ${rule} should be in the TS config`);
            }
        });
        it("base rules should appear in both JS and TS configs", () => {
            ...
            for (const rule of baseRules) {
                assert.ok(rule in jsRules, `base rule ${rule} should be in the JS config`);
                assert.ok(rule in tsRules, `base rule ${rule} should be in the TS config`);
            }
        });
    });
});
describe("recommendedWithLocalesEn config", () => {
    ...
    describe("structure", () => {
        ...
        it("sentence-case-json should be 'warn' for en.json files", () => {
            assert.strictEqual(getSeverity(enJsonRules["obsidianmd/ui/sentence-case-json"]), "warn");
        });
        it("sentence-case-locale-module should be 'warn' for en.ts files", () => {
            assert.strictEqual(getSeverity(enTsRules["obsidianmd/ui/sentence-case-locale-module"]), "warn");
        });
        it("sentence-case-json should be off or absent for non-en files", () => {
            ...
            assert.ok(severity === "off" || !("obsidianmd/ui/sentence-case-json" in tsRules), `sentence-case-json should be off or absent for non-en files, got: ${severity}`);
        });
        it("type-checked rules should not appear in JS config", () => {
            for (const rule of TYPE_CHECKED_RULES) {
                assert.ok(!(rule in jsRules), `type-checked rule ${rule} should NOT be in the JS config for recommendedWithLocalesEn`);
            }
        });
        it("type-checked rules should appear in TS config", () => {
            for (const rule of TYPE_CHECKED_RULES) {
                assert.ok(rule in tsRules, `type-checked rule ${rule} should be in the TS config for recommendedWithLocalesEn`);
            }
        });
    });
});
describe("scanner-aligned severities", () => {
    ...
    it("security rules should be at error", () => {
        ...
        for (const rule of securityRules) {
            assert.strictEqual(getSeverity(tsRules[rule]), "error", `security rule ${rule} should be 'error'`);
        }
    });
    it("scanner-escalated obsidianmd rules should be at error", () => {
        ...
        for (const rule of escalatedRules) {
            assert.strictEqual(getSeverity(tsRules[rule]), "error", `scanner-escalated rule ${rule} should be 'error'`);
        }
    });
    it("scanner-escalated type-checked rules should be at error", () => {
        ...
        for (const rule of escalatedTypeChecked) {
            assert.strictEqual(getSeverity(tsRules[rule]), "error", `scanner-escalated type-checked rule ${rule} should be 'error'`);
        }
    });
    it("non-escalated obsidianmd rules should be at warn", () => {
        ...
        for (const rule of warnRules) {
            assert.strictEqual(getSeverity(jsRules[rule]), "warn", `non-escalated rule ${rule} should be 'warn'`);
        }
    });
    it("non-escalated type-checked rules should be at warn", () => {
        ...
        for (const rule of warnTypeChecked) {
            assert.strictEqual(getSeverity(tsRules[rule]), "warn", `non-escalated type-checked rule ${rule} should be 'warn'`);
        }
    });
    it("eslint-comments rules should be at error", () => {
        ...
        for (const rule of eslintCommentsRules) {
            assert.strictEqual(getSeverity(tsRules[rule]), "error", `eslint-comments rule ${rule} should be 'error'`);
        }
    });
    it("rule-custom-message should be at error", () => {
        assert.strictEqual(getSeverity(tsRules["obsidianmd/rule-custom-message"]), "error", "rule-custom-message should be 'error'");
    });
});
describe("type-checked rule guard", () => {
    ...
    it("type-requiring rules must not appear in JS config", () => {
        for (const rule of TYPE_CHECKED_RULES) {
            assert.ok(!(rule in jsRules), `${rule} requires type info (getParserServices) but appears in JS config — move it to recommendedPluginRulesConfigTypeChecked only`);
        }
    });
    it("type-requiring rules must appear in TS config", () => {
        for (const rule of TYPE_CHECKED_RULES) {
            assert.ok(rule in tsRules, `${rule} requires type info but is missing from TS config — add it to recommendedPluginRulesConfigTypeChecked`);
        }
    });
});
